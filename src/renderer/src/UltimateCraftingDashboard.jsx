...
export default function UltimateCraftingDashboard() {
  ...
  // 1. โหลดข้อมูลเริ่มต้นและเปิดระบบฟังเสียง Realtime จาก Supabase
  useEffect(() => {
    fetchInitialData();

    // สร้างช่องทางรับข้อมูล Realtime
    const channel = supabase
      .channel('schema-db-changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'server_configs' }, (payload) => {
        // เมื่อมีใครแก้ข้อมูลบนฐานข้อมูล โค้ดนี้จะทำงาน
    ...
    return () => {
      supabase.removeChannel(channel); // ปิดการเชื่อมต่อเมื่อปิดโปรแกรม
    };
  }, []);

  const fetchInitialData = async () => {
    try {
      const { data, error } = await supabase.from('server_configs').select('data').eq('id', 1).single();
      
      if (data && data.data) {
        setCities(data.data);
        if (data.data.length > 0) setActiveCityId(data.data[0].id);
      } else {
        // ถ้าฐานข้อมูลเพิ่งสร้างใหม่ (ยังไม่มีข้อมูล) ให้ดัน defaultData เข้าไป
        await supabase.from('server_configs').upsert({ id: 1, data: defaultData });
  ...
  // 2. ฟังก์ชันอัปเดตข้อมูลขึ้น Supabase
  const syncToCloud = async (newCitiesData) => {
    setCities(newCitiesData); // อัปเดตจอเราทันทีให้ดูไว
    // ยิงขึ้นคลาวด์ (id: 1 คือแถวแรกที่เราใช้เก็บข้อมูลเมืองทั้งหมด)
    await supabase.from('server_configs').upsert({ id: 1, data: newCitiesData });
  };

  // ดึงข้อมูลเก่าจาก Local Storage ขึ้น Supabase
  const handleMigrateOldData = async () => {
    const saved = localStorage.getItem('craftingProData');
    if (saved) {
      const localData = JSON.parse(saved);
      if (window.confirm('พบข้อมูลเก่าในเครื่องนี้! ต้องการอัปโหลดขึ้น Supabase เพื่อทับข้อมูลปัจจุบันไหม?')) {
        await syncToCloud(localData);
        alert('✅ ดึงข้อมูลเก่าขึ้น Supabase สำเร็จแล้ว!');
      }
    } else {
  ...
  if (!isLoaded) return <div style={{ height: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', background: '#0b0f19', color: '#fff' }}>Connecting Supabase... ⚡</div>;
        ...
        <div className="sidebar">
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '11px', color: '#10b981' }}>● Supabase Online</div>
            </div>
          </div>
